...
1. **具象有温度**：把抽象概念转化为有人间烟火气的可视意象，拒绝「精致但空洞」的物体。核心问题，这张图如果不告诉我配哪篇文章，我能猜出来吗？猜不出来就是空洞。
   - ❌「一把精致的金属尺子，尺面刻着数字，末端溶解消散在雾气中」（漂亮但失重，和经济无关的图一样可以用）
   - ✅「一个戴老花镜的会计在昏黄台灯下翻账本，旁边摆着算盘和一叠发票，但窗外街道上的小摊贩、路边的人群完全在视野之外」（用「看不见的东西」来具象化「统计盲区」，且场景唯一）
2. **叙事瞬间优先**：图要暗示「一个正在发生的时刻」，而不是「一个静态的物体」。
   - ❌「a broken chain」（静态物体，无叙事）
   - ✅「the moment a chain link snaps under tension, one end swinging loose, blurred motion, dramatic side light」（叙事瞬间，有力量感）
3. **有使用痕迹**：道具要有被用过、被人碰过的痕迹。新的东西没有故事，旧的才有。scratched / worn / dog-eared / coffee-stained / crumpled 这类词要主动用。
4. **情绪传达**：提示词要包含情绪方向词。不只描述"什么东西"，还要描述"什么感觉"，anxious / quietly defiant / hollow triumph / tender exhaustion 等。
5. **风格一致**：每张图的提示词末尾都必须追加全局风格锚中的参数
6. **避免文字**：提示词中明确声明 --no text, letters, words, characters（AI 生图的文字几乎都是乱码）
7. **避免人脸**：除非文章明确需要人物肖像，否则用剪影、背影、局部特写代替正脸（避免恐怖谷效应和版权问题）
8. **中英双语**：主提示词用英文（AI 生图工具对英文理解更好），附中文说明说明视觉意图
...
### I5 空洞陷阱检查（新增必检项）
对每张图独立回答，有任何一项「否」则必须重写提示词：
- 这张图如果不说是给哪篇文章配的，能猜出来吗？（不能 = 太空洞）
- 图里有没有「被用过的痕迹」的细节？（没有 = 太精致太空）
- 提示词描述的是一个「正在发生的时刻」还是「一个静态物体」？（纯物体 = 缺叙事感）
